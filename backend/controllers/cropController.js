...
const getRecommendations = (req, res) => {
  ...
  FarmProfile.findByFarmerId(farmer_id, (err, data) => {
    ...
    const { profile_id, region, soil_type, farm_size, water_availability } =
      data[0];
    ...
    Crop.getRecommended(
      ...
      (err, crops) => {
        ...
        if (crops.length === 0)
          return res.json({ Error: "No matching crops found" });
...
const getSavedRecommendations = (req, res) => {
  ...
  FarmProfile.findByFarmerId(farmer_id, (err, data) => {
    ...
    Crop.getByProfile(data[0].profile_id, (err, crops) => {
      if (err)
        return res.json({ Error: "Error getting saved recommendations" });
      return res.json({ Status: "Success", crops });
    });
  });
};
